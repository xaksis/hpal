  ...
  public function run($request, $response) {
    ...
    if (empty($purpose) || $purpose == 'signin') {
      ...
      if (!empty($inputEmail) && ($inputEmail != $verifiedEmail)) {
        ...
        if (empty($account)) {
          $this->action->saveAssertion($request, $response);
          //$user = $this->action->showNewAccountPage($request, $response);
			$user = array();
			$user['email'] = $assertion->getVerifiedEmail();
			$user['displayName'] = $assertion->getDisplayName();
			$user['fullName'] = $assertion->getFullName();
			$user['photoUrl'] = $assertion->getPhotoUrl();
			$user['firstName'] = $assertion->getfirstName();
			$user['lastName'] = $assertion->getLastName();
			//echo var_dump($user); 
			gitContext::getAccountService()->addUser($user);
			$account = gitContext::getAccountService()->getAccountByEmail($verifiedEmail);
			$this->mergeAccountInfo($account, $assertion);
			$request->setAccount($account);
